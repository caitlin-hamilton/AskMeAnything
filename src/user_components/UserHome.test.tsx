...
describe('HomeTestComponent', () => {
    test('Render voted or not voted on click', () => {
        const wrapper = mount(<UserHome getQuestions={getTestData} getUserData={getVoteData} userId={"9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d"}></UserHome>)
...
describe('HomeTestComponentReactTestingLibrary', () => {
    test('User has already voted so decrement', () => {
        const { getByTestId } = render(<UserHome getQuestions={getTestData} getUserData={getVoteData} userId={"9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d"}></UserHome>)
    ...
    test('User has not already voted so increment', () => {
        const { getByTestId } = render(<UserHome getQuestions={getTestData} getUserData={getVoteData} userId={""}></UserHome>)
        const { getByText } = within(getByTestId('num-of-votes'))
        expect(getByText('5')).toBeInTheDocument()
